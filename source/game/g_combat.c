...
{//drains DP from victim.  Also awards experience points to the attacker.
	...
	}/* else if(victim->client->ps.stats[STAT_DODGE] > 120) {
      victim->client->ps.stats[STAT_DODGE] = 120; //Magic number, turn into variable later.
   }*/

	if(attacker && attacker->client && amount > 0)
