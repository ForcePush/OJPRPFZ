...
void Cmd_SetDP(gentity_t *ent)
{
	char idStr[256] = { 0 };
	char maxDodgeStr[256] = { 0 };
	...
	if (trap_Argc() < 3)
	{
		trap_SendServerCommand(ent->playerState->clientNum,
			"print \"^3Usage: ^5/setDP <client id> <amount>\n\"");
		return;
	}
	...
	g_entities[id].playerState->stats[STAT_MAX_DODGE] = maxDodge;
	g_entities[id].playerState->stats[STAT_DODGE] = maxDodge;
}

void Cmd_SetFP(gentity_t *ent)
{
	char idStr[256] = { 0 };
	char maxFPStr[256] = { 0 };
	...
	{
		trap_SendServerCommand(ent->playerState->clientNum,
			"print \"^3Usage: ^5/setFP <client id> <amount>\n\"");
		return;
	}
	...
	g_entities[id].playerState->fd.forcePowerMax = maxFP;
	g_entities[id].playerState->fd.forcePower = maxFP;
}
/*
==================
Cmd_God_f
	...
	else if (Q_stricmp(cmd, "grantadmin" ) == 0)
	{
      ...
      { 
         ...
         { 
			...
			else if (Q_stricmp(cmd, "setDP") == 0)
			{
				Cmd_SetDP(ent);
			}
			else if (Q_stricmp(cmd, "setFP") == 0)
			{
				Cmd_SetFP(ent);
			}
	else
