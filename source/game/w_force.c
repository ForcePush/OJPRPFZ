...
void WP_SpawnInitForcePowers( gentity_t *ent )
{
	...
	if (ent->client->ps.fd.forcePowerMax == 0)  // spawned just now
	{
	...
	else if (ent->client->ps.fd.forcePowerMax == 100 &&  // normal force power
		ent->client->ps.fd.forcePowerLevel[FP_SEE] >= FORCE_LEVEL_3)
	{
		ent->client->ps.fd.forcePowerMax = 115;
	}
	else if (ent->client->ps.fd.forcePowerMax == 115 &&  // normal force power + FP_SEE
		ent->client->ps.fd.forcePowerLevel[FP_SEE] < FORCE_LEVEL_3)
	{
...
int ForcePowerUsableOn(gentity_t *attacker, gentity_t *other, forcePowers_t forcePower)
{
	...
	if(forcePower == FP_TELEPATHY && other->client)
	{
		...
		case FORCE_LEVEL_3:
            ...
            if (other->client->ps.stats[STAT_DODGE] < DODGE_CRITICALLEVEL ||
                other->client->ps.saberAttackChainCount >= MISHAPLEVEL_HEAVY)
                return 1;
...
{//generically checks to see if the defender is able to block an attack from this attacker 
	...
	if (attackPower == FP_GRIP)
	{ // Все уже проверено, до нас...
		return qfalse;
	}

    if (attackPower == FP_PUSH ||
        attackPower == FP_PULL)
	{
        if (defender->client->ps.stats[STAT_DODGE] < DODGE_CRITICALLEVEL ||
			defender->client->ps.saberAttackChainCount >= MISHAPLEVEL_HEAVY)
                return qfalse;
	}
...
qboolean ValidGripEnt(gentity_t*self,gentity_t*ent)
{
	...
	// Skinpack: do not check absorb again
	...
	*/
		...
		)
	{
		...
		{//you have enough fatigue to transfer to Dodge
			...
			self->client->ps.fd.forcePower--; // Skinpack: WTF is this shit?
		}
